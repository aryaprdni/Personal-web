...
app.post('/add-project', upload.single('image'), requireLogin, addProject);
...
async function home(req, res) {
    const query = `SELECT projects.id, projects.name, projects.start_date, projects.end_date, projects.description, projects.image, projects.technologies, users.name AS author, projects."createdAt", projects."updatedAt" FROM projects INNER JOIN users ON projects."authorId" = users.id`;
    ...
    console.log("ini data project dari database", projects);
...
async function detail(req, res) {
    ...
    const query = `SELECT projects.id, projects.name, projects.start_date, projects.end_date, projects.description, projects.image, projects.technologies, users.name AS author, projects."createdAt", projects."updatedAt" FROM projects INNER JOIN users ON projects."authorId" = users.id WHERE projects.id=${id}`;
    const project = await sequelize.query(query, { type: QueryTypes.SELECT });
    const technologies = {
